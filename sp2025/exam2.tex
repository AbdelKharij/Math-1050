...
\begin{document}
\begin{abstract}
Exam 2, covering Chapter 2: veto power and pivotal players, counting coalitions,
Banzhaf power and dummies, reconstructing a system's winning coalitions from its
pivots, and Shapley-Shubik power in a law firm.
\end{abstract}
\maketitle
...
\begin{prompt}
\textbf{(a)} Claim: in a weighted voting system, it is possible for every player to
have veto power.
...
\begin{feedback}
True. If the quota equals the total weight, as in $[10:4,3,2,1]$, only the unanimous
coalition wins, so any single player can block a motion.
\end{feedback}
\end{prompt}

\begin{prompt}
\textbf{(b)} Claim: in a weighted voting system, a player has veto power if they appear
in every \emph{winning} coalition.
...
\begin{feedback}
True. If every winning coalition contains the player, then no coalition can win without
them --- which is exactly what it means to be able to veto.
\end{feedback}
\end{prompt}

\begin{prompt}
\textbf{(c)} Claim: in a weighted voting system, it is possible for every player in a
\emph{sequential} coalition to be pivotal.
...
\begin{feedback}
False. Each sequential coalition has exactly one pivotal player: the first one whose
arrival brings the running total up to the quota.
\end{feedback}
\end{prompt}
\end{problem}
