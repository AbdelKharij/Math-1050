...
\begin{document}
\begin{abstract}
The cumulative final: which apportionment methods suffer which paradoxes
and quota violations, saddle points, a full four-method apportionment of teaching
aides, the subtraction game, and expected value as a decision tool.
\end{abstract}
\maketitle
...
1(b) --- can be pushed below their lower quota. For example, a district with a standard quota of $86.915$ (lower quota $86$) can
receive only $85$ seats under Adams' method.
...
\begin{prompt}
\textbf{(a)} Alice and Bob are playing a variation of the Subtraction Game: start with
$13$ stones, each turn a player may take $1$, $2$, $3$, or $4$ stones, and the winner
takes the final stone. If Alice goes first, who has a winning strategy?
...
How many stones should she take on her first turn? $\answer{3}$

\begin{feedback}
With moves of $1$ to $4$, the losing positions are the multiples of $5$. Since $13$ is
not one, Alice wins: she takes $3$, leaving $10$, and afterwards answers each of Bob's
moves of $k$ stones with $5-k$, so Bob always faces a multiple of $5$ --- and finally
$0$.
\end{feedback}
\end{prompt}

\begin{prompt}
\textbf{(b)} Complete the payoff matrix from Alice's perspective for a two-player
...
\begin{prompt}
\textbf{(c)} Alice is deciding whether to go to a basketball game that her friend Jalen
