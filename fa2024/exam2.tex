...
\begin{prompt}
\textbf{(b)} Claim: in a weighted voting system, a player has veto power \emph{if and only
if} they appear in every winning coalition.
...
\begin{feedback}
True, in both directions. If every winning coalition contains the player, no coalition
can win without them, so they can veto. Conversely, if they can veto, every coalition
without them loses, so every winning coalition contains them.
\end{feedback}
\end{prompt}
