...
\begin{document}
\begin{abstract}
Practice for Exam 2, covering Chapter 2: veto power and dictators, counting coalitions,
Banzhaf power and dummies, reconstructing winning coalitions from pivots, and
Shapley-Shubik power in a law firm.
\end{abstract}
\maketitle
...
\begin{prompt}
\textbf{(a)} Claim: in a weighted voting system, it is possible for a player with weight
...
\begin{feedback}
True. Veto power only requires that everyone else together falls short of the quota. In
$[10:4,3,2,1]$ the player with weight $1$ --- far below half of $10$ --- can block any
motion.
\end{feedback}
\end{prompt}

\begin{prompt}
\textbf{(b)} Claim: in a weighted voting system, if one player's weight is greater than
the combined weight of all other players, then that player is a dictator.
...
\begin{feedback}
False. A dictator's weight must meet the \emph{quota} on its own. In $[10:6,1,1,1]$, the
first player outweighs everyone else combined ($6>3$) but $6<10$, so they cannot pass
anything alone. (They do have veto power, since the others total only $3$.)
\end{feedback}
\end{prompt}

\begin{prompt}
\textbf{(c)} Claim: in a weighted voting system, it is possible for every player in a
winning coalition to be critical.
...
\begin{feedback}
True. In a coalition that only just reaches the quota, losing anyone drops it below ---
for example $\set{B,C,D}$ in $[12:8,6,4,2]$, with weight exactly $12$.
\end{feedback}
\end{prompt}
\end{problem}
...
\begin{prompt}
\textbf{(c)} In the system $[13:6,5,4,3,2,1]$ with six players, how many sequential
coalitions are there?
...
$B$, $C$, and $D$. Each partner has one vote, and decisions can be made by a simple
majority or by the senior partner with any one junior partner's support. Use $J$ for
any one junior partner.

\begin{prompt}
\textbf{(a)} There are four sequential coalition types, depending on where A falls. In
...
\[
\begin{array}{ll}
\seq{A,\underline{J},J,J} & \text{A with one junior wins}\\
\seq{J,\underline{A},J,J} & \text{same}\\
\seq{J,J,\underline{A},J} & \text{two juniors are not a majority; A completes it}\\
\seq{J,J,\underline{J},A} & \text{three of four is a majority}
\end{array}
\]
\end{feedback}
\end{prompt}
...
\begin{prompt}
\textbf{(b)} How many sequential coalitions are there of each type, and in total?

Each type: $\answer{6}$ \qquad Total: $\answer{24}$
...
each: $\frac{4}{24}\approx 16.67\%$. Compare Homework 4, where the same firm's
\emph{Banzhaf} distribution also gave the senior partner $50\%$.
\end{feedback}
\end{prompt}
\end{problem}
