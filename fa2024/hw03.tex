...
\begin{problem}
Table 1 shows the preference schedule for Best Sandwich Shop in town.
...
\begin{problem}
Find \emph{two} examples of an election where a \emph{Condorcet candidate} does not win
(if you can't find two, you are welcome to make up your own). For each, write out the
preference schedule and the tabulation method used, then identify which candidate is
the Condorcet candidate and which candidate wins under that method.
...
\begin{feedback}
Many examples work. Here is a small one using Plurality with three candidates and $9$ voters:
...
\textbf{A second example, using the Borda Count}, with $5$ voters:
\begin{center}
\begin{tabular}{c|cc}
Preference & 3 & 2 \\\hline
1st & A & B \\
2nd & B & C \\
3rd & C & A
\end{tabular}
\end{center}
A is the Condorcet candidate: A beats B $3$--$2$ and beats C $3$--$2$ (A even has a
majority of first-place votes). But the Borda scores, with $3$, $2$, $1$ points, are
A $=3\cdot 3+2\cdot 1=11$, B $=3\cdot 2+2\cdot 3=12$, C $=3\cdot 1+2\cdot 2=7$ --- so
\textbf{B wins the Borda Count}.

\textbf{What these imply:} both Plurality and the Borda Count violate the
\emph{Condorcet criterion}, which says that if a Condorcet candidate exists, that
candidate should win. Plurality ignores everything below first place; Borda can let a
broadly acceptable candidate outscore one that a majority prefers head-to-head.
